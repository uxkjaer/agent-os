...
if [ "$VSCODE" = true ]; then
    ...
    if [ "$IS_FROM_BASE" = true ]; then
        ...
        # Download from GitHub when using --no-base
        echo "  Downloading VSCode files from GitHub..."
        echo ""
        echo "  📂 Commands:"
        for cmd in plan-product create-spec create-tasks execute-tasks analyze-product; do
            download_file "${BASE_URL}/claude-code/${cmd}.md" \
                "./.github/instructions/${cmd}.instructions.md" \
                "false" \
                "instructions/${cmd}.instructions.md"
        done
...
if [ "$VSCODE" = true ]; then
    echo "   .github/instructions/          - Github CoPilot Instructions"
    echo "   .github/agents/            - Github CoPilot specialized agents"
fi
